...
class CountryFourChoicesButtonCommandHandler : ButtonCommandService {

    override val enum get() = CountryButtonCommand.FOUR_CHOICES
    ...
    private fun getShowAnswer(cbQuery: CallbackQuery): SendMessageResponse {
        ...
        val lastCountryAnswer = getLastCountryAnswer(userId, isCorrectAnswer, cbQuery)
        ...
        if (isCorrectAnswer) {
            ...
            return SendMessageResponse(listOf(lastCountryAnswer, nextFlagQuestion))
        } else {
            return SendMessageResponse(listOf(lastCountryAnswer))
        }
    }
